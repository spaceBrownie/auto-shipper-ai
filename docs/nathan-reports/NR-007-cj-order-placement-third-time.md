...
The result: 46 files changed, 42 real tests (zero fake ones), all passing — including full integration tests against the actual database. The Unblocked code reviewer caught one additional edge case that all 42 tests missed (more on that below), which was fixed and verified before merging. One review cycle, clean merge.
...
- **Idempotency**: Duplicate events (network retries, system restarts) don't produce duplicate CJ orders — the system recognizes it already placed the order and skips
- **Supplier abstraction**: CJ is the first supplier, but the system is structured so adding Printful, Printify, or any other dropship supplier means plugging in a new integration, not rearchitecting
...
2. System receives the webhook, creates the order ✓ (FR-023, already done)
...
The Unblocked code reviewer flagged a scenario none of the 42 tests covered: what happens if a CJ order fails, and then something retries the placement?

The system checks "has a supplier order already been placed?" by looking for a CJ order ID on the internal order. But a *failed* order has no CJ order ID — it was never placed successfully. So the idempotency check passes, the system tries CJ again, and then tries to mark the order as "failed" when it's *already* failed. The system doesn't allow transitioning from "failed" to "failed" — it crashes.

This wouldn't happen today (nothing retries failed orders yet), but it's exactly the kind of time bomb that detonates six months later during a 2am incident. The fix was a five-line guard: only attempt supplier placement on confirmed orders. Two regression tests lock it down.

The takeaway: automated tests cover the scenarios you think of. External review covers the ones you don't. Both are essential.
...
- **Supplier product mapping population** — The system knows *how* to map a SKU to a CJ product variant, but the mapping table needs data. This happens when the first real product is selected for launch.
...
- Fixed 5 pre-existing data corruption bugs in the Shopify webhook parser as a bonus — same null-string issue from Attempt 2, but in fields that existed before this feature. These would have been silent until real orders with missing phone numbers or empty apartment fields hit the system.
- Full test suite runs against a real PostgreSQL database — not in-memory fakes. This caught an integration issue (the new event chain changed order status in a way that broke an existing test) that simpler tests would have missed entirely.
