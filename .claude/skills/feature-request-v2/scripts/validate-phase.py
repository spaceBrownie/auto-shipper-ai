#!/usr/bin/env python3
"""
Feature Request Phase Validation Script (v2 — 6-phase workflow)
...
Usage:
    ...
    python validate-phase.py --phase 5 --action bash --command "git commit"
    ...
    python validate-phase.py --phase 5 --bulk-validate "src/main/kotlin/Foo.kt" "src/main/kotlin/Bar.kt" --json
"""
...
class PhaseValidator:
    ...
    def check_deliverables(self, phase_number: int, feature_dir: str) -> Tuple[bool, List[str]]:
        ...
        for deliverable in deliverables:
            ...
            if deliverable_type == 'file':
                ...
                # Phase 5 specific: check all checkboxes are complete
                if phase_number == 5 and file_name == 'implementation-plan.md':
                    with open(file_path, 'r') as f:
                        content = f.read()
        ...
        if phase_number == 5:
            ...
            # Enforce phase-5 decision support artifacts, if configured
    ...
    def format_phase_info(self, phase_number: int, as_json: bool = False) -> str:
        ...
        if requires_approval:
            output.append("  Manual approval required before proceeding")
...
def main():
    parser = argparse.ArgumentParser(description='Validate feature request workflow phase actions (v2 — 6-phase)')
    ...
    parser.add_argument('--workflow-config', type=str,
                        default='.claude/skills/feature-request-v2/references/feature-workflow.yaml',
    ...
    if args.feature_name:
        ...
        if args.json:
            ...
            print(f"{'OK' if valid else 'FAIL'} {message}")
        ...
        # Bulk validate write permissions for multiple paths
        if not args.phase:
            print("Error: --phase required for bulk validation", file=sys.stderr)
            sys.exit(1)

        results = []
        all_valid = True
        for path in args.bulk_validate:
            valid, message = validator.validate_write(args.phase, path)
            results.append({"path": path, "valid": valid, "message": message})
            if not valid:
                all_valid = False

        if args.json:
            print(json.dumps({"all_valid": all_valid, "results": results}))
        else:
            for r in results:
                print(f"{'OK' if r['valid'] else 'FAIL'} {r['path']}: {r['message']}")
        ...
        if args.json:
            ...
            if complete:
                print(f"OK All deliverables complete for phase {args.phase}")
            else:
                print(f"FAIL Deliverable errors for phase {args.phase}:")
                for error in errors:
                    print(f"  - {error}")
        ...
        if args.json:
            ...
            if complete:
                print(f"OK Decision-support checks complete for phase {args.phase}")
            else:
                print(f"FAIL Decision-support errors for phase {args.phase}:")
                for error in errors:
                    print(f"  - {error}")
        sys.exit(0 if complete else 1)
