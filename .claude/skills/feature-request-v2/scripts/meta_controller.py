#!/usr/bin/env python3
"""
Brain-inspired meta-controller for feature workflow strategy selection (v2 — 6-phase).
...
def phase_allows_parallel_agents(state: DecisionState, workflow_config_path: str) -> bool:
    ...
    Default behavior:
    - Parallel execution is only considered in phases 5 (implementation) and 6 (review-fix).
    - Phases 1-4 block parallel agents (discovery, spec, planning, test-first are single-agent).
    - If validator is available, verify phase can write Java/Kotlin source.
    """
    if state.phase not in (5, 6):
        return False
...
def generate_recommendations(
    ...
    if state.phase == 5 and not reward["parallel_preference_enabled"]:
...
def decide(
    state: DecisionState,
    workflow_config_path: str = ".claude/skills/feature-request-v2/references/feature-workflow.yaml",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Brain-inspired strategy selector for feature workflow agents (v2 — 6-phase)."
    )
    parser.add_argument(
        "--state-file",
        type=str,
