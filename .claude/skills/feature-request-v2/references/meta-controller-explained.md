...
- **Anterior cingulate-like conflict/cost monitoring (functional analogy)**
  ...
  System analog: explicit cost terms (`time`, `compute`, `risk`, `merge`) and adaptive weights (`lambda`) that increase penalties when risk or urgency is high.
...
- **Control-theory view:** hard feasibility filter (`Action in AllowedSet(phase)`)
...
`Score(a|x) = Q(a|x) - lambda_t*T(a|x) - lambda_c*C(a|x) - lambda_r*R(a|x) - lambda_m*M(a|x) + Bparallel(a|x)`
...
- `lambda_*`: context-sensitive penalty weights derived from urgency/risk/coupling
...
Assume a phase-5 state with moderately parallelizable work:

- `p=0.8`, moderate coupling/novelty
- derived weights: `lambda_t=0.22`, `lambda_c=0.11`, `lambda_r=0.30`, `lambda_m=0.14`
...
`delta = Score3 - Score1 = 0.0188`

Decision rule:

- if `delta > parallel_margin_min` (default `0.012`), choose parallel (`n=3`)
...
`U(k) = P(success|k)*Vsuccess - lambda_t*PlanningOverhead(k)`
`VOC(k) = U(k) - U(k-1)`
...
`Rbase = Q - lambda_t*T - lambda_c*C - lambda_r*R`

Shaped reward:

`Rshaped = Rbase + beta * I(parallel AND quality>=floor AND rework<=ceiling)`
...
2. In non-implementation phases (1-4), parallel worker expansion is blocked.
3. In phase 4 (test-first gate), parallel execution is blocked; test generation runs single-agent.
4. In phase 5, state is inferred from `implementation-plan.md` unless explicitly overridden.
5. In phases 5 and 6, full candidate search is enabled (except high-risk entanglement fallback to single mode).
...
| - Write test-manifest.md + test files    |
| - Single-agent only (no parallel)        |
| - Confirm all tests red                  |
+------------------------------------------+
         |
         v
 [Manual Approval #4 — test approval]
         |
         v
+-----------------------------------------+
...
[Fix preflight]  +----------------------------------------------+
   ...
   +------------>| 1) single vs parallel agents                 |
         ...
         |
         v
[Execute tasks by dependency group + update checkboxes + tests]
         |
         v
[Create summary.md]
         |
...
| - Fix regressions / flaky tests          |
| - Code quality review                    |
+------------------------------------------+
         |
         v
   [All tests green + no findings?]
      /         \
    No           Yes
    |             |
    v             v
...
flowchart TD
    ...
    M3 -->|Approved| P4[Phase 4: Test-First Gate]

    P4 --> V4[validate-phase.py checks<br/>Single-agent only]
    V4 --> D4[Write test-manifest.md + test files<br/>Confirm all tests red]
    D4 --> M4{Manual Approval #4<br/>Test approval}
    M4 -->|Approved| P5[Phase 5: Implementation]

    P5 --> G[Hard Gate: validate-phase.py<br/>Allowed read/write/bash only]
    ...
    E1 -->|parallel| X2[Orchestrator + parallel agents<br/>dependency-ordered groups]
    ...
    X1 --> RUN[Execute tasks by dependency group]
    X2 --> RUN
    X3 --> RUN
    X4 --> RUN
    X5 --> RUN
    ...
    S --> P6[Phase 6: Review-Fix Loop]

    P6 --> REV[Full test suite + review scan]
    REV --> CHK{All green + no findings?}
    CHK -->|No| FIXR[Fix regressions / issues]
    FIXR --> REV
    CHK -->|Yes| Z[Complete]
