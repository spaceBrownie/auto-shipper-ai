...
```bash
# Use the project-local venv (installed from spaceBrownie/graphify fork)
PYTHON=".graphify-venv/bin/python3.14"
$PYTHON -c "import graphify" 2>/dev/null || { echo "ERROR: graphify venv not found. Run: python3 -m venv .graphify-venv && source .graphify-venv/bin/activate && pip install git+https://github.com/spaceBrownie/graphify.git"; exit 1; }
# Write interpreter path for all subsequent steps
$PYTHON -c "import sys; open('.graphify_python', 'w').write(sys.executable)"
```

If the import succeeds, print nothing and move straight to Step 2. **Do NOT install from PyPI — always use the project venv.**
