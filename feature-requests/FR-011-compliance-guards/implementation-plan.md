...
- [x] Update `ComplianceFailed` event — kept `reason: String` with `.name` serialisation (avoids shared coupling to compliance internals)
...
### Domain Layer
- [x] Implement `ComplianceFailureReason` enum with 6 reasons
- [x] Implement `ComplianceCheckResult` sealed class (`Cleared`, `Failed`) with `ComplianceCheckType` enum
- [x] Implement `ComplianceAuditRecord` JPA entity (skuId, checkType, result, reason, checkedAt, detail)
...
- [x] Implement `SourcingCheckService.check(skuId, vendorId)` (validate vendor status via native query against vendors table — cross-module boundary pattern)
- [x] Implement `ComplianceOrchestrator` running all 4 checks concurrently with `supervisorScope { async {} }`
...
### Proxy Layer
- [ ] Implement `ClaudeComplianceAdapter` calling Claude API for IP/claims analysis (toggled by `compliance.llm.enabled`) — deferred to Phase 2
- [ ] Implement `SanctionsListCache` loading from local file, refreshing daily at midnight — deferred to Phase 2
...
### Config Layer
- [x] Define `ComplianceConfig` `@ConfigurationProperties` (`llm.enabled`, `auto-check.enabled`, `sanctions-list.path`, `prohibited-categories.path`)
