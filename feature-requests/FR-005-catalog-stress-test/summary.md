...
4. **Duplicate `SkuTerminated` event in fail path**: `StressTestService` explicitly published `SkuTerminated` after calling `skuService.transition(…, Terminated)`, but `SkuService.transition()` already publishes that event internally (lines 49-51). Removed the redundant `eventPublisher.publishEvent(SkuTerminated(...))` call and the `ApplicationEventPublisher` constructor dependency from `StressTestService`. Updated `StressTestServiceTest` to remove the stale mock/verify and replace it with a verify on `skuService.transition(skuId, Terminated(STRESS_TEST_FAILED))`.
