...
6. **Cross-module event listener transaction pattern** — All `@TransactionalEventListener(phase = AFTER_COMMIT)` handlers that write to the database must also carry `@Transactional(propagation = Propagation.REQUIRES_NEW)`. Without `REQUIRES_NEW`, writes are silently discarded because Spring's `TransactionSynchronizationManager` retains stale state in the post-commit callback. Plain `@EventListener` is acceptable only for same-module listeners that should rollback with the publisher. **Additionally**, webhook-triggered `AFTER_COMMIT` handlers (where the publishing transaction serves an HTTP response to an external system) must also carry `@Async`. Without `@Async`, `AFTER_COMMIT` fires synchronously in the commit thread — the entire processing chain (DB lookups, external API calls with retry backoff) blocks the HTTP response. PM-015 Bug #5 documents this: Shopify's 5-second timeout was exceeded because the processing listener ran synchronously. The full annotation stack for webhook-triggered listeners is `@Async` + `@TransactionalEventListener(phase = AFTER_COMMIT)` + `@Transactional(propagation = Propagation.REQUIRES_NEW)`.
...
16. **@Entity classes with assigned (non-generated) @Id must implement `Persistable<T>`** — Spring Data's `SimpleJpaRepository.save()` checks `isNew()` to choose between `persist()` and `merge()`. For non-`Persistable` entities, `isNew()` returns `false` when the ID is non-null — which is always true for assigned IDs (`UUID.randomUUID()`, external strings). This forces `merge()`, which issues a SELECT before INSERT and defers the flush. In `REQUIRES_NEW` transactions, the deferred flush means `DataIntegrityViolationException` fires after the catch block, breaking deduplication. Implement `Persistable<T>` with a `@Transient private var isNew: Boolean = true` field, `@PostPersist`/`@PostLoad` to flip it to `false`. Enforced by ArchUnit Rule 3 in `ArchitectureTest`.
17. **All Jackson `get()?.asText()` calls on external JSON payloads must use the NullNode guard** — `?.let { if (!it.isNull) it.asText() else null }`. Plain `?.asText()` returns `"null"` (the string) for JSON `null` values (`NullNode.asText()` returns `"null"`, not Kotlin null). Apply the guard consistently — if one field in a block uses it, all fields must. Constraint 15 covers `get()` vs `path()` selection; this constraint covers the NullNode trap after `get()` returns a non-null `NullNode`.
18. **Never implement features in a single unstructured session ("vibe coding")** — PM-008 and PM-010 document how single-session implementation without the feature-request workflow produces green builds with broken features: silent data loss, missing transaction boundaries, untested error paths, and constraint violations that unit tests cannot catch. Always use the 6-phase feature-request workflow (`.claude/skills/feature-request-v2/`). If time constraints prevent full workflow execution, explicitly document which phases were skipped and what risks that introduces.
19. **Try-catch after dedup commit in event listeners** — Any `@TransactionalEventListener(AFTER_COMMIT)` handler that processes a deduplicated event (where the dedup record was committed in a prior transaction) MUST wrap its critical operations in try-catch. Without this, a transient failure (DB connection, optimistic lock, external API) leaves the event permanently stuck — retries are blocked by the committed dedup record returning `"already_processed"`. Log at ERROR with the dedup key for alerting and manual intervention. PM-015 Bugs #1/#2 and PM-019 Bug #5 document this pattern.
20. **Never create bare `ObjectMapper()` in Spring components** — Always inject the Spring-managed `ObjectMapper` bean, which has the Kotlin module (`jacksonObjectMapper`) and other project-configured modules registered. A bare `ObjectMapper()` silently produces incorrect JSON for Kotlin data classes (missing default values, wrong nullability). In tests that construct adapters directly, pass `ObjectMapper()` or `jacksonObjectMapper()` explicitly — but production code must always use the injected bean.
